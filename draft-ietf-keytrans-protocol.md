...
until the log roughly doubles in size and a new root is established at entry 63.

The protocol frequently refers to the **frontier** of the log, which is defined
...
example of a log with 50 entries, the frontier would be entries: `31, 47, 49`.
...
Users reuse the implicit binary search tree from {{implicit-binary-search-tree}}
...
expects label owners to perform monitoring.
...
3. If the given log entry has a left child in the implicit binary search tree,
   ...
   log entry.
4. If the given log entry has a right child, recurse to its subtree by executing
   this algorithm with: the given log entry's right child, the timestamp of the
   given log entry, and the given right timestamp.
...
To perform a search, users need to be able to inspect individual log entries and
...
version of the label was greater than, equal to, or less than their **target
version**.
...
called **search binary ladders** and follow the series of lookups described in
...
When searching the combined tree structure described in {{combined-tree}} for a
...
have been verified by the label owner (discussed in {{monitoring-the-tree}}).

## Maximum Lifetime

A Transparency Log operator MAY define a maximum lifetime for log entries. If
defined, it MUST be greater than zero and greater than the RMW. Whether a log
...
ways: The user may have inspected a log entry which is **not** expired and
...
6. If this step is reached, the search has terminated without finding an
   ...
   2. If any expired log entries were encountered in the search, identify
      ...
      been checked for correctness by the label owner.
...
3. For each log entry in the computed list, from left to right:
   ...
   1. Check if a binary ladder from this log entry was already provided in the
      ...
      2. If it had a target version less than or equal to that of the current
         map entry, terminate and return an error to the user.
...
This algorithm works by progressively moving up the implicit binary search tree
...
If any new versions of the label were created after the requested starting
position, the label owner will need to process each of these individually as
described in {{updating-a-label}}.

Once the label owner has fully initialized their state, through the algorithm
...
4. If a stop condition has been reached, stop. From a user's perspective, the
   ...
   by the user, or if a maximum output size has been reached.
...
Since this algorithm checks only distinguished log entries for correctness,
label owners use it in conjunction with the algorithm in {{contact-algorithm}}
...
3. If the log entry where the new versions were added is distinguished in the
   ...
   `PrefixProof` from the log entry for those versions in ascending order by
...
4. If the log entry where the new versions were added is not distinguished in
   the current tree, obtain a `PrefixProof` from it with lookups corresponding
   ...
   label being the greatest that exists.
   ...
   a second `PrefixProof` from the log entry with those lookups in ascending
   order by version. Verify that all lookups result in an inclusion proof.
...
In essence, this algorithm has two phases. The first phase is doing a
...
- For each log entry touched by the algorithm in {{fv-algorithm}}:
  ...
  - If the log entry is not expired, then a `PrefixProof` corresponding to a
    search binary ladder in the log entry's prefix tree.
- If step 6.3 is reached, a second `PrefixProof` from the identified log entry
  specifically looking up the target version.

### Contact Monitoring

For a user to monitor a label in the combined tree, the following is provided:

- For each entry in the user's monitoring map, from rightmost to leftmost log
  entry:
  - The timestamp of each log entry on the path from the root to the parent of
    the log entry in the user's monitoring map, stopping if a non-distinguished
    log entry is established.
  - From each log entry in the list computed in {{contact-algorithm}}, a
    `PrefixProof` corresponding to a monitoring binary ladder.
...
- The timestamp of each log entry on the path from the root to the user's
  requested starting position.
- For each log entry in the list computed in the first algorithm of
  {{owner-algorithm}}, a `PrefixProof` corresponding to a search binary ladder.

The timestamps are provided to allow both the client and server to verify the
...
- The timestamp for each log entry that causes the second algorithm of
  {{owner-algorithm}} to recurse either left or right.
- For each log entry that reaches step 5 in the second algorithm of
  {{owner-algorithm}}, a `PrefixProof` corresponding to a search binary ladder.
...
- The timestamp of each log entry on the path from the root to the parent of the
...
- The timestamp of each log entry on the path from the root to the parent of the
  ...
  non-distinguished log entry is established.
...
Users MUST retain the most recent `TreeHead` and `AuditorTreeHead` they've
successfully verified as part of any query response, and populate the `last`
field of any subsequent query request with the `tree_size` of the last
`TreeHead`. This ensures that all operations performed by the user return
consistent results.
...
3. Compute the VRF output for each version of the label from the proofs in
   `binary_ladder`. Compute the commitment to the value of the target version
   with `opening` and `value`.
4. If `SearchRequest.version` is absent, verify the proof in `search` as
   described in {{gv-algorithm}}. Otherwise, verify the proof in `search` as
   described in {{fv-algorithm}}.
5. Compute a candidate root value for the tree from the proof in
...
If the Transparency Log is deployed in Contact Monitoring mode, users that
looked up a label initiate a Contact Monitoring operation by submitting a
`ContactMonitorRequest` to the Transparency Log containing information about the
label they wish to monitor.
...
The Transparency Log verifies that each `MonitorMapEntry` in `entries` is sorted
in ascending order by `position`, that no `position` or `version` is duplicate,
and that `position` always lies on the direct path of the first log entry to
contain the associated version of the label. In turn, the Transparency Log
responds with a `ContactMonitorResponse` structure:
...
{{contact-algorithm}}. Users verify a `ContactMonitorResponse` by following
these steps:

1. Verify the proof in `monitor` as described in {{contact-algorithm}}.
...
of a Transparency Log deployed in Contact Monitoring mode MUST be able to
...
would be for them to provide the corresponding commitment opening.

## Owner Initialization

Users that wish to initialize their owner state for a label submit an
`OwnerInitRequest` to the Transparency Log containing the label they wish to
take ownership of and their desired starting log entry.
...
The Transparency Log verifies that `start` identifies a log entry that is
unexpired and distinguished. In turn, the Transparency Log responds with an
`OwnerInitResponse` structure:
...
The `greatest_versions` field contains the greatest version of the label that
exists in each log entry computed in step 1 of the first algorithm in
{{owner-algorithm}}, ending at the first log entry where the label doesn't
exist.

The `binary_ladder` field contains one entry for each version computed in step 3
of the first algorithm in {{owner-algorithm}}, in ascending order by version. If
a version of the label exists, the commitment to the label's value at that
version is provided in the `commitment` field of the `BinaryLadderStep`.

The `init` field contains the output of updating the user's view of the tree to
match `TreeHead.tree_size` followed by executing the first algorithm in
{{owner-algorithm}}.

Users verify an `OwnerInitResponse` by following these steps:

1. Verify that the number of entries in `greatest_versions` is less than or
   equal to the number of log entries in the computed list, and that
   `greatest_versions` is in descending order.
2. Verify that the expected number of entries is present in `binary_ladder` and
   that a commitment is provided for each version in `greatest_versions`. (Note
...
3. Verify the proof in `init` as described in the first algorithm of
   {{owner-algorithm}}.
4. Compute a candidate root value for the tree from the proof in
   `init.inclusion` and any previously retained full subtrees of the log tree.
5. With the candidate root value for the tree, verify `FullTreeHead` as
   described in {{full-tree-head-verification}}.

## Owner Monitoring

Users that wish to monitor a label they own submit an `OwnerMonitorRequest` to
...
The `entries` list contains the label owner's monitoring state for versions of
...
1. Verify that each MonitorMapEntry in `entries` is sorted in ascending order by
...
The `monitor` field contains the output of updating the user's view of the tree
to match `TreeHead.tree_size`, followed by executing the algorithm in
{{contact-algorithm}}, followed by executing the second algorithm in
{{owner-algorithm}}. Users verify an `OwnerMonitorResponse` by following these
steps:

1. Verify the proof in `monitor` as described in {{contact-algorithm}} and the
   second algorithm of {{owner-algorithm}}.
2. Compute a candidate root value for the tree from the proof in
   `monitor.inclusion` and any previously retained full subtrees of the log
   tree.
3. With the candidate root value for the tree, verify `FullTreeHead` as
...
Users that wish to create one or more new versions of a label they own submit an
`UpdateRequest` to the Transparency Log containing the label and the new values
to store.
...
If a Third-Party Manager is present, the `skipped_versions` field contains the
number of version counters that were skipped due to desynchronization between
the Service Operator and the Third-Party Manager (discussed in {{management}}).
Skipped versions are added to the prefix tree with an all-zero commitment value,
...
non-empty, this indicates that `UpdateRequest.values` was disregarded by the
Transparency Log. Inversely, if `UpdateResponse.values` is empty, this indicates
that all of the new versions of the label in `UpdateRequest.values` were
successfully created and no versions of the label other than the indicated
number of skipped versions, followed by those in `UpdateRequest.values`, were
created in the log entry `position`.

The `info` field contains an `UpdateInfo` for each new and unskipped version of
the label created in the log entry `position`. The entries of `info` correspond
to the entries of `UpdateResponse.values` if it is non-empty, and to the entries
of `UpdateRequest.values` otherwise.
...
to match `TreeHead.tree_size`, followed by executing the algorithm in
{{update-algorithm}}. Users verify an `UpdateResponse` by following these steps:
...
2. If `UpdateResponse.values` is non-empty, verify that the length of `info` is
...
`position` where more new versions of the label were inserted. These
`UpdateResponse` structures are processed by clients serially as if an
`UpdateRequest` with the following parameters had been sent:
...
1. Verify that the user is aware of a recently issued distinguished log entry at
   `position`. To preserve the efficiency of the protocol and (potentially) the
   sender's anonymity, implementations MUST NOT use a verification failure at
   this point as a prompt to request updates from the Transparency Log.
...
of the label in the indicated log entry.
...
Users follow these steps to *initially verify* a provisional credential:

1. Verify that `TreeHead.tree_size` is greater than `position`.
2. Verify the proof in `search` as described in {{gv-algorithm}}. Verify that
   the terminal log entry of the search is to the right of `position`.
3. Compute a candidate root value for the tree from the proof in
   `search.inclusion` and the retained full subtrees of the log tree.
4. With the candidate root value for the tree, verify `TreeHead.signature` as
   described in {{tree-head-signature}}.

Once initially verified, the contents of a credential can be consumed by the
application. The initial verification process explicitly rejects a provisional
credential if a standard credential could have been produced from the same log
entry `position`. However, when generating a credential, users MAY choose to
output a provisional credential from an older distinguished log entry rather
than a standard credential from a newer distinguished log entry. This can allow
users to accommodate reasonable delays in the distribution of newly issued
...
1. From the proof in `Credential.search`, compute the prefix tree root value of
   the terminal log entry. Verify that this matches the user's retained state
   (learned from making a `DistinguishedRequest` or through owner monitoring)
   for the distinguished log entry at that position.

Alternatively, the terminal log entry may never become distinguished. In this
case, the position of the first distinguished log entry issued to the right of
the terminal log entry is denoted `distinguished`. Once this distinguished log
entry is established, the sender of the credential provides a `CredentialUpdate`
structure:
...
The proof in `monitor` contains the result of updating the user's view of the
tree from a size of `Credential.tree_head.tree_size` to a size of
`distinguished+1`. If the Transparency Log is deployed in Contact Monitoring
mode, the proof in `monitor` then contains the output of executing the algorithm
in {{contact-algorithm}}, which will follow the monitoring path from the
terminal log entry to the log entry at `distinguished`.

Users follow these steps to verify a `CredentialUpdate`, which finally verifies
the provisional credential if successful:
...
1. Verify that the user is aware of a recently issued distinguished log entry
   ...
   terminal log entry. Again, a verification failure at this point MUST NOT
   prompt a request for an updated tree head from the Transparency Log.
2. Verify the proof in `monitor` as described in {{update-view-algorithm}} and,
   if the Transparency Log is deployed in Contact Monitoring mode, as described
   in {{contact-algorithm}}.
3. Compute a candidate root value for the log tree with the proof in
   `monitor.inclusion` and the full subtrees of the tree at size `tree_size` (as
   computed from the proof in `Credential.search`). Verify that this matches the
   user's retained state for the distinguished log entry at `distinguished`.

Verifying a provisional credential or its associated `CredentialUpdate` MUST NOT
have any effect on the state used for the user's direct interactions with the
Transparency Log, or on the verification of other credentials (even for the same
label). In particular, the view of the tree presented in a provisional
credential MUST NOT cause a user to change its view of the tree for any other
purpose.

To minimize the potential for issues caused by clock skew, users SHOULD support
receiving a `CredentialUpdate` before they're capable of verifying it (due to
not knowing the necessary distinguished log entry yet) by queueing it for later
verification. If a user initially verifies a provisional credential and is
unable to finally verify it before the credential expires, the user MUST attempt
to obtain and verify a `CredentialUpdate` from the Transparency Log directly.
The user SHOULD attempt to fetch the `CredentialUpdate` over an anonymous
channel, to preserve the anonymity of the sending user as much as possible. If
the user is still unable to obtain or verify a `CredentialUpdate` structure for
a provisional credential, the credential SHOULD be reported to the user as
evidence of potential misbehavior by the Transparency Log.
...
When a `CredentialUpdate` is received, the proof it contains extends the view of
the tree presented in the credential to a size of `distinguished+1`. Users are
able to compare this with their locally retained view of the tree at that size,
ensuring that neither this view nor the one presented in the credential are
inconsistent with the other views received from the Transparency Log directly.
...
greatest version of the label to be created is `signed_version`, before
inserting the versions requested in the `ManagerUpdateRequest`. These dummy
entries, which map the VRF output for each label-version pair to an all-zero
commitment, serve the function of skipping the corresponding version counters.

If `signed_version` is less than the next greatest version of the label to be
created, this strongly indicates a serious bug in the Service Operator and the
request MUST be rejected by the Manager.
